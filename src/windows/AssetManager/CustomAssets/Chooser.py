...
class CustomAssetChooser(ChooserPage):
    ...
    def on_video_toggled(self, button):
        settings = gl.settings_manager.load_settings_from_file(os.path.join(gl.UI_SETTINGS_PATH, "AssetManager.json"))
        settings["video-toggle"] = button.get_active()
        gl.settings_manager.save_settings_to_file(os.path.join(gl.UI_SETTINGS_PATH, "AssetManager.json"), settings)
